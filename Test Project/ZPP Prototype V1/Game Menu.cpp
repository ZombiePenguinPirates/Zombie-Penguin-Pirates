...
void initMenu()
{
	...
	MainMenuScreen.SetPos(0, 0);
	//and added the render function to end of drawgame
	MainMenuScreen.SetState(main_menu);
...
state RunMenu(POINT mousepos, bool Lmousedown, bool Rmousedown)
{
	...
	case in_game_pause_quit:
		...
		break;
	case GameOver:
		if(MouseMenu(UserInterface[30], mousepos, Lmousedown, Rmousedown)&&click_press)
		{
				option = UserInterface[30].Getstate();
				click_press = false;
				//return option
				return option;
		}
...
{
	MainMenuScreen.Render(pBuffer);			//load game menu
...
{
	MainMenuScreen.Render(pBuffer);			//setting menu
...
{
	MainMenuScreen.Render(pBuffer);			//tutorials menu
...
{
	MainMenuScreen.Render(pBuffer);			//quit menu
...
void drawMenu8(BUFFER &pBuffer)
{
	...
	UserInterface[30].Render(pBuffer);
}